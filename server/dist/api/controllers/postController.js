...
export const editComment = async (req, res) => {
    ...
    try {
        ...
        const response = await Post.updateOne({ _id: postID, "comments._id": _id }, { $set: { "comments.$": editedComment } });
    ...
    try {
        ...
        if (editedComment.author !== uid)
            return res
                .status(500)
                .json({ message: "Failed to edit comment: User is not the author." });
        const response = await Post.updateOne({ _id: postID }, { $pull: { comments: { _id } } });
        const newPost = await Post.findById(postID);
        res.status(200).json(newPost);
    }
    catch (error) {
...
export const togglePostLike = async (req, res) => {
    ...
    try {
        ...
        if (isLiked)
            post.likes = post.likes.filter((user) => user !== uid);
        else
            post.likes.push(uid);
        await post.save();
        res.status(200).json(post);
